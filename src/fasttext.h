...
namespace fasttext {
    ...
    protected:
        ...
        // initOutputBeforeTraining() replace std::shared_ptr<Matrix> createTrainOutputMatrix() const;
    ...
    public:
        ...
        void predict(int32_t k, const type_words & words, Predictions & predictions, real threshold = 0.0) const;

        bool predictLine(std::istream& in, type_proba2groupLabel &proba2groupLabel, int32_t k, real threshold) const;
        /**
         * @param in:  stream txt file one text per line
         * @param proba2groupLabel: output group
         * @param k>0: nombre max de prédictions retenue, post condition predictions.size() <= k
         * @param threshold: loss function pour prendre en compte le groupe
         * @return vrai si des données on été prédites
         */
        ...
        /**
         * precondition: this->arg_ initialised, requis pour Classifier.cc l'interfacage python de talk4
        */
        void initDictionnaryBeforeTraining(const Args& args);
