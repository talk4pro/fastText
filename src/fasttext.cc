...
namespace fasttext {
    ...
    void FastText::predict(int32_t k, const type_words & words, Predictions& predictions, real threshold) const {
    ...
    bool FastText::predictLine(std::istream& in, type_proba2groupLabel &proba2groupLabel, int32_t k, real threshold) const{

        proba2groupLabel.clear();
        if (in.peek() == EOF)  return false;

        type_words words, labels;
        ...
        for (const auto& p : linePredictions) {
            proba2groupLabel.push_back( std::make_pair(std::exp(p.first), dict_->getLabel(p.second)) );
        }
    ...
    void FastText::predictGroupNumber(std::istream &in, int32_t k,
                                      type_proba2groupNumber &predictions) const {
        ...
        Predictions linePredictions;
        predict(k, words, linePredictions, 0.);

        std::transform( linePredictions.cbegin(), linePredictions.cend(), std::back_inserter(predictions),
                [](const Prediction &p){ return std::make_pair( std::exp(p.first), p.second);} );

    }

/*
    void FastText::predictGroupLabels(std::istream &in, int32_t k,
                                      FastText::type_proba2grouLabel &predictions) const {

        predictions.clear();

        type_proba2groupNumber modelPredictions;
        predictGroupNumber(in, k, modelPredictions);
    ...
    void FastText::displayPredict(std::istream &in, int32_t k, bool print_prob) {
        std::vector<std::pair<real, std::string>> predictions;
        while (in.peek() != EOF) {
            predictGroupLabels(in, k, predictions);
...
// depreacted, use getAnalogies instead
    ...
    void FastText::initDictionnaryBeforeTraining(const Args& args){

        if (args.input == "-") {
            // manage expectations
            throw Exception("Cannot use stdin for training!");
        }

        args_ = std::make_shared<Args>(args);
        if (args.d_dict.get() != nullptr){
        ...
        if (!ifs.is_open()) {
            ...
            message = std::string("Input file ") + args.input + " cannot be opened!";
    ...
    void FastText::initInputBeforeTraining(){
        input_ = (args_->pretrainedVectors.empty())? createRandomMatrix(): getInputMatrixFromFile(args_->pretrainedVectors);
    }


    void FastText::initOutputBeforeTraining(){
        int64_t m = (args_->model == model_name::sup) ? dict_->nlabels() : dict_->nwords();
        auto output = std::make_shared<DenseMatrix>(m, args_->dim);
        output->zero();
        output_ = output;
    }
    ...
    void FastText::train(const Args& args) {
        ...
        initDictionnaryBeforeTraining(args);
        initInputBeforeTraining();
        initOutputBeforeTraining();
