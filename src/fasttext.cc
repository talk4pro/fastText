    ...
    namespace fasttext {
        ...
        void FastText::supervised(
            ...
            if (labels.empty() || line.empty() == 0) return;
        ...
        void FastText::predictGroupNumber(std::istream &in, int32_t k,
                                          FastText::type_proba2groupNumber &modelPredictions, real threshold) const {
            assert(modelPredictions.empty());
        ...
        void FastText::predictGroupLabels(std::istream &in, int32_t k,
                                          FastText::type_proba2grouLabel &predictions, real threshold) const {

            type_proba2groupNumber modelPredictions;
            predictGroupNumber(in, k, modelPredictions, threshold);
        ...
        void FastText::displayPredict(std::istream &in, int32_t k, bool print_prob, real threshold) {
            std::vector<std::pair<real, std::string>> predictions;
            while (in.peek() != EOF) {
                predictions.clear();
                predictGroupLabels(in, k, predictions, threshold);
        ...
        void FastText::initDictionaryBeforeTraining(){
            ...
            if (!ifs.is_open()) {
                ...
                message = std::string("Input file ") + args_->input + " cannot be opened!";
        ...
        void FastText::saveModelAfterTraining(){
            saveModel();
            if (args_->model != model_name::sup) {
                saveVectors();
