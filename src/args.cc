...
namespace fasttext {
    ...
    void Args::setDefaultValues(const char *argv_command){
        lr = 0.05;
        dim = 100;
        ws = 5;
        epoch = 5;
        ...
        if (argv_command == nullptr) return;

        std::string command(argv_command);
        if (argv_command == "supervised") {
        ...
        else if (argv_command == "cbow") {
            model = model_name::cbow;
        }
    }

    Args::Args() {
        setDefaultValues("supervised");
    }

    Args::Args(int argc, char **argv) {
        setDefaultValues(argv[1]);
