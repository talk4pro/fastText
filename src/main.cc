...
void printUsage() {
  std::cerr
      ...
      << "  test_google                    evaluate a supervised classifier\n"
      << "  test_google-label              print labels with precision and recall scores\n"
...
void printTestUsage() {
  std::cerr
      << "usage: fasttext test_google <model> <test_google-data> [<k>] [<th>]\n\n"
      << "  <model>      model filename\n"
      << "  <test_google-data>  test_google data filename (if -, read from stdin)\n"
...
void printPredictUsage() {
  std::cerr
      << "usage: fasttext predict[-prob] <model> <test_google-data> [<k>] [<th>]\n\n"
      << "  <model>      model filename\n"
      << "  <test_google-data>  test_google data filename (if -, read from stdin)\n"
      << "  <k>          (optional; 1 by default) predict top k labels\n"
      << "  <th>         (optional; 0.0 by default) probability threshold\n"
...
void printTestLabelUsage() {
  std::cerr
      << "usage: fasttext test_google-label <model> <test_google-data> [<k>] [<th>]\n\n"
      << "  <model>      model filename\n"
      << "  <test_google-data>  test_google data filename\n"
      << "  <k>          (optional; 1 by default) predict top k labels\n"
      << "  <th>         (optional; 0.0 by default) probability threshold\n"
      << std::endl;
}
...
void test(const std::vector<std::string>& args) {
  bool perLabel = args[1] == "test_google-label";
